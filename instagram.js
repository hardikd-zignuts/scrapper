...
app.get('/user-details', async (req, res) => {
    ...
    try {
        ...
        // Close the browser
        await browser.close();
        res.json(elementsContent || { error: 'Numbers not found' });
    } catch (error) {
...
app.get('/profile-image', async (req, res) => {
    try {
        ...
        await page.waitForSelector(`img[alt="${username}\'s profile picture"]`, { timeout: 5000 }).catch(() => { });

        // Extract the image URL using page.evaluate
        let imageUrl = await page.evaluate((username) => {
            const imgElement = document.querySelector(`img[alt="${username}\'s profile picture"]`);
            if (imgElement) {
